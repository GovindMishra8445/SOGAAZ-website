...
const tx = {
  en: {
    ...
    btn1: "Invite SOGAZ to a tender",
    btn2: "Become a SOGAZ agent",
    bottomLinks: [
    ...
    copyright:
      "© 2026 Joint-Stock Company 'Insurance Company of the Gas Industry' (JSC 'SOGAZ'). All rights reserved. The information posted on this website is not a public offer as defined by clause 2 of Article 437 of the Civil Code of the Russian Federation.",
  ...
  ru: {
    ...
    careers: {
      title: "Работа в СОГАЗе",
      links: ["Вакансии", "Отправить резюме", "Рекомендовать специалиста"],
    },
    dms: {
      title: "Для застрахованных по ДМС",
      phones: ["8 800 333 44 19", "8 495 780 78 80", "8 495 956 44 19"],
    },
    qrText: "Наведите камеру смартфона, чтобы скачать приложение",
    phone: "8 800 333 08 88",
    phoneLabel: "Единый контактный центр",
    btn1: "Пригласить СОГАЗ на конкурс",
    btn2: "Стать агентом СОГАЗ",
    bottomLinks: [
    ...
    copyright:
      "© 2026 Акционерное общество «Страховое общество газовой промышленности» (АО «СОГАЗ»). Все права защищены. Информация, размещённая на настоящем интернет-сайте, не является публичной офертой, установленной в п. 2 ст. 437 ГК РФ.",
...
// Simple QR placeholder (real QR should be an image)
const QRPlaceholder = () => (
  <div className="w-[110px] h-[110px] bg-white rounded-xl flex items-center justify-center flex-shrink-0">
    <svg width="90" height="90" viewBox="0 0 90 90" fill="none">
      {/* QR visual pattern */}
      <rect x="2" y="2" width="30" height="30" rx="3" fill="#0A1172" />
      <rect x="8" y="8" width="18" height="18" rx="1" fill="white" />
      <rect x="12" y="12" width="10" height="10" rx="1" fill="#0A1172" />
      <rect x="58" y="2" width="30" height="30" rx="3" fill="#0A1172" />
      <rect x="64" y="8" width="18" height="18" rx="1" fill="white" />
      <rect x="68" y="12" width="10" height="10" rx="1" fill="#0A1172" />
      <rect x="2" y="58" width="30" height="30" rx="3" fill="#0A1172" />
      <rect x="8" y="64" width="18" height="18" rx="1" fill="white" />
      <rect x="12" y="68" width="10" height="10" rx="1" fill="#0A1172" />
      {/* center dots */}
      {[38, 44, 50, 56, 62, 38, 44, 50, 56, 62].map((x, i) => (
        <rect
          key={i}
          x={x}
          y={i < 5 ? 38 : 50}
      ...
      <rect x="38" y="44" width="4" height="4" fill="#0A1172" />
      <rect x="50" y="44" width="4" height="4" fill="#0A1172" />
      <rect x="62" y="44" width="4" height="4" fill="#0A1172" />
    </svg>
  </div>
);
...
const Footer = () => {
  ...
  return (
    <footer className="bg-[#0A1172] text-white mt-16">
      {/* ── TOP SECTION ── */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 pt-12 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[auto_1fr_1fr_1fr_auto] gap-10">
          {/* Logo + lang */}
          ...
          <div className="flex flex-col items-start lg:items-end gap-3">
            <QRPlaceholder />
            <p className="text-xs text-white/50 max-w-[120px] text-left lg:text-right leading-relaxed">
              {t.qrText}
            </p>
          </div>
        </div>
      ...
      {/* ── COPYRIGHT ── */}
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
