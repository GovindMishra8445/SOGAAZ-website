...
const MobileMenu = ({ onClose }) => {
  ...
  const currentMenu = activeTab === "private" ? menuData : businessMenuData;

  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col overflow-y-auto">
      {/* Header — X at same left position as hamburger */}
      <div className="flex items-center justify-between  py-4 border-b border-gray-100">
        <button
          onClick={onClose}
          className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-[#F1F2F5] transition-colors"
        >
          <X size={22} color="#0A1172" />
        </button>

        <Link to="/" onClick={onClose}>
          <svg width="80" height="26" viewBox="0 0 120 36" fill="none">
            <text
              x="0"
              y="28"
        ...
        {/* SOS button */}
        <button className="flex items-center gap-1.5 px-4 py-2 bg-[#F1F2F5] hover:bg-[#09D171] rounded-full text-sm font-bold text-gray-800">
      ...
      {/* Tab switcher */}
      <div className="flex px-5 pt-5 border-b border-gray-100">
        <button
          onClick={() => {
                    ...
                    <ul className="space-y-1">
                      {section.items.map((item, k) => (
                        ...
                        >
                          ...
                          {item.tag && (
                            <span className={tagStyles[item.tag]}>
                              {item.tag}
                            </span>
                          )}
                        </li>
      ...
      <div className="px-4 py-6 space-y-3">
        ...
        <div className="pt-1">
          ...
          <a
            ...
            className="font-bold text-lg text-[#0A1172]"
          >
            {tx.phone}
          </a>
        </div>
